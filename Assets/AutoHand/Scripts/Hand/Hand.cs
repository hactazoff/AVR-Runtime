            ...
            if(enableMovement) {
                if(holdingObj && !holdingObj.maintainGrabOffset && !IsGrabbing()) {
                    ...
                    if(!holdingObj.useGentleGrab) {
                        GetGrabTime();
                        grabPositionOffset = Vector3.MoveTowards(grabPositionOffset, Vector3.zero, grabReturnPositionDistance.magnitude * (Time.deltaTime / GetGrabTime()) / 2f + deltaDist * Time.deltaTime * 90f);
                        grabRotationOffset = Quaternion.RotateTowards(grabRotationOffset, Quaternion.identity, grabReturnRotationDistance.eulerAngles.magnitude * (Time.deltaTime / GetGrabTime()) + (deltaRot)/ 2f * Time.deltaTime * 90f);
                    }
                }
                else if(!holdingObj && !IsGrabbing()) {
                    grabPositionOffset = Vector3.MoveTowards(grabPositionOffset, Vector3.zero, Time.deltaTime * 60f);
                    grabRotationOffset = Quaternion.RotateTowards(grabRotationOffset, Quaternion.identity, Time.deltaTime * 360f);
                }
                ...
                if(grabType == GrabType.HandToGrabbable || (grabType == GrabType.GrabbableToHand && (holdingObj.HeldCount() > 0 || !holdingObj.parentOnGrab))) {
                    ...
                    for(float i = 0; i < adjustedGrabTime; i += Time.deltaTime) {
                        if(holdingObj != null) {
                            ...
                            var deltaDist = Vector3.Distance(follow.position, lastFrameFollowPos);
                            float maxDeltaTimeOffset = minGrabTime/adjustedGrabTime * Time.deltaTime;
